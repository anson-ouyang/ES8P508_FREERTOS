...
/***************************************************************
  函数名：CCMConfig
  描  述：外部晶振停振检测配置
  输入值：ie:中断使能, en:检测使能, intType:中断触发方式
  输出值：无
  返回值：无
 ***************************************************************/
void CCMConfig(TYPE_FUNCEN ie, TYPE_FUNCEN en, CCM_TYPE_TRIG intType)
{
    SCU_RegUnLock();                        //解锁
    SCU->CCM.EN = en;
    SCU->CCM.IE = ie;
...
/***************************************************************
  ...
  返回值：无
 ***************************************************************/
FlagStatus IsXTALStopped()
{
		FlagStatus status = RESET;
    SCU_RegUnLock();                        //解锁
    status = (FlagStatus)(SCU->CCM.FLAG);
    SCU_RegLock();
		return status;
}
